...
export default async function StudentTreasuryPage({ searchParams }: { searchParams?: { status?: string } }) {
  ...
  return (
    <DashboardLayout
      ...
      title="Tesouraria & Pagamentos"
      subtitle="Gerencie suas faturas e recibos"
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <BentoCard className="bg-gradient-to-br from-navy to-blue-600 text-white md:col-span-2" dark>
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-blue-100">Saldo Devedor Total</p>
            ...
            {pendingInvoices.length > 0 ? (
              <>
                <h3 className="mt-2 text-2xl font-black text-rose-500">
                  {pendingInvoices[0].dueDate.toLocaleDateString("pt-PT")}
                </h3>
              ...
              <h3 className="mt-2 text-2xl font-black text-emerald-500 text-sm">Tudo em dia!</h3>
            )}
          </BentoCard>
        </div>
        ...
        <BentoCard className="p-0">
          <div className="p-6 border-b border-slate-100 flex items-center justify-between">
            <h3 className="text-lg font-black text-slate-900">Histórico Financeiro</h3>
            <StatusBadge tone="navy">{student?.invoices.length ?? 0} Transações</StatusBadge>
          </div>
          <DataTable
            emptyMessage="Nenhuma fatura encontrada."
            headers={["Referência", "Data", "Vencimento", "Valor", "Estado", "Ações"]}
            rows={(student?.invoices ?? []).map((invoice) => [
              <span key={invoice.id} className="font-black text-slate-700">{invoice.reference}</span>,
              invoice.createdAt.toLocaleDateString("pt-PT"),
              invoice.dueDate.toLocaleDateString("pt-PT"),
